 ...
 *
 * @param int    $post_id Post ID.
 * @param string $context single|card|public|modal.
 */
function one1_render_story_donation_form( $post_id, $context = 'single' ) {
	$post_id = (int) $post_id;
	if ( ! in_array( $context, array( 'single', 'public', 'modal' ), true ) || ! one1_story_is_donation( $post_id ) || ! one1_has_donation_payment_form() ) {
		return;
	}
