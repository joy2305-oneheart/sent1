 ...
 * @param array<string, mixed> $meta    Story meta.
 * @param string               $context card|single|public|modal.
 */
function one1_render_story_donation_summary( $post_id, $meta, $context = 'card' ) {
	...
	>
		...
		<div class="sent-share-donation__cta">
			<?php
			$has_form = in_array( $context, array( 'single', 'public', 'modal' ), true )
				&& function_exists( 'one1_has_donation_payment_form' )
				&& one1_has_donation_payment_form();

			if ( $has_form ) {
				$form_collapsed = 'modal' === $context;
				?>
				<button
					type="button"
					class="one-story-donation-open"
					data-one-donation-open
					data-story-id="<?php echo esc_attr( (string) $post_id ); ?>"
					<?php echo $form_collapsed ? '' : 'hidden'; ?>
				>
					<span class="material-symbols-outlined" aria-hidden="true">volunteer_activism</span>
					<?php esc_html_e( 'Donate now', 'one' ); ?>
				</button>
				<div
					class="one-story-donation-form-wrap<?php echo $form_collapsed ? '' : ' is-open'; ?>"
					data-one-donation-form-wrap
					<?php echo $form_collapsed ? 'hidden' : ''; ?>
				>
					<?php one1_render_story_donation_form( $post_id, $context ); ?>
				</div>
				<?php
			} else {
